...
import { MoodIcon, ArrowLeft, Loader2, Star, Music, Heart } from './components/Icons';

const App: React.FC = () => {
  const [selectedMood, setSelectedMood] = useState<MoodType | null>(null); // The finalized mood triggering the message
  const [pickerMood, setPickerMood] = useState<MoodType>(MoodType.SAD); // The mood currently highlighted in the picker
  const [message, setMessage] = useState<GeneratedMessage | null>(null);
  const [loading, setLoading] = useState(false);
  const [isTransitioning, setIsTransitioning] = useState(false);

  // Scroll ref to potentially auto-scroll to selected item (optional polish)
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden font-sans text-slate-700 bg-slate-50">
      ...
      <main className={`relative z-10 w-full max-w-2xl px-6 py-8 transition-all duration-500 ease-in-out ${isTransitioning ? 'opacity-0 scale-95' : 'opacity-100 scale-100'}`}>
        
        {!selectedMood ? (
          // HOME VIEW - CLEAN SCROLL PICKER
          <GlassCard className="p-10 md:p-14 text-center animate-float flex flex-col items-center min-h-[500px] justify-center">
                    ...
                    {Object.values(MoodType).map((mood) => (
                      <button
                        ...
                        className={`
                          ...
                          ${pickerMood === mood 
                            ? 'bg-lavender-100/50 text-lavender-600 scale-100 font-semibold shadow-sm backdrop-blur-sm' 
            ...
            <button
              ...
              className="
                ...
                hover:shadow-lg hover:shadow-lavender-300/50 hover:-translate-y-1
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-r from-lavender-400 to-lilac-400 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
            </button>

          </GlassCard>
        ) : (
          // MESSAGE VIEW (Unchanged logic, just keeping it here)
          <GlassCard className="p-8 md:p-16 min-h-[500px] flex flex-col items-center justify-center text-center">
            
            {loading ? (
              <div className="flex flex-col items-center animate-fade-in">
                <div className="relative">
                    <div className="w-16 h-16 rounded-full border-2 border-lavender-200 border-t-lavender-500 animate-spin"></div>
                    <div className="absolute inset-0 flex items-center justify-center">
                        <MoodIcon mood={selectedMood} className="w-6 h-6 text-lavender-400 opacity-50" />
                    </div>
                </div>
              ...
              <div className="w-full animate-fade-in">
                 <button 
                    onClick={handleReset}
                    className="absolute top-6 left-6 text-slate-400 hover:text-lavender-600 transition-colors p-2 rounded-full hover:bg-white/50"
                    aria-label="Back"
                  >
                ...
                <div className="mb-6 flex justify-center">
                  <div className="p-3 bg-lavender-50 rounded-full text-lavender-500 shadow-inner">
                ...
                {message?.playlist && message.playlist.length > 0 && (
                  <div className="mb-8 mx-auto max-w-sm">
                    <div className="flex items-center justify-center gap-2 text-lavender-500 mb-3 opacity-80">
                ...
                <div className="w-16 h-px bg-gradient-to-r from-transparent via-lavender-300 to-transparent mx-auto mb-6"></div>
      ...
      <footer className="fixed bottom-4 w-full text-center z-10">
        <p className="text-xs text-slate-400/60 font-sans tracking-widest">
          MADE FOR ZOLOO
        </p>
      </footer>
    </div>
